...
2026-08-21에는 `--display` 대조 모드로 강남역 2호선과 고속터미널역 3호선에서 각각 7개의
방향·종착역별 마지막 출발 행을 생성했다. 이 모드는 원본 응답 전문을 출력하지 않고 앱 표시용
비민감 필드만 보여주며, 공휴일 판정 상태도 함께 검사한다.

같은 날 공공데이터포털 `Encoding` 키를 기존 Free 플랜 Worker Secret에 반영했다. 이중 인코딩
회귀를 수정하고 테스트 18개를 통과한 뒤 재배포했으며, 아래 안전한 점검기로 `/health`, 무인증
401, 2026-08-21 평일과 2026-08-15 광복절 판정을 종단간 확인했다.
