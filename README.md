...
- 앱 전면에서만 동작하는 40초 간격 자동 갱신, 오류 시 최대 5분 백오프와 30분 세션 제한
...
- 홈·실시간·막차·역을 분리한 4탭 구조와 빠른 실행 버튼 중심의 Morning Transit 디자인
- 밝은 미스트 블루 배경, 기능별 파스텔 카드, 노선 고유색, 큰 도착 예정시간과 LIVE/지연 상태
- 즐겨찾기와 최근 역을 기기에 저장하고 검색 결과에서 홈으로 바로 전환하는 역 탐색 흐름
- 기기 로컬 막차 알림 예약·해제와 출발 20분 전 또는 임박 시 5분 전 안내
...
- Worker 경로별 요청 제한, 20초 실시간 캐시·요청 합치기, 6시간 시간표 캐시
- 역 데이터 생성·검증 스크립트, iOS XCTest 47개, Worker 회귀 테스트 22개
...
요청과 중복 호출하지 않습니다. 연속 오류가 나면 자동 간격은 80초·160초·최대 5분으로
늘어나며, 한 번의 전면 실행에서 30분이 지나면 자동 호출을 멈춥니다. 화면의 새로고침을 누르면
새 30분 세션을 시작합니다.
...
Morning Transit 디자인은 홈·실시간·막차·역의 네 탭으로 정보량을 분리합니다. 홈은 지금 탈
열차와 다음 도착, 데이터 신뢰 상태를 요약하고, 실시간 탭은 현재 역 기반 열차 위치, 막차 탭은
예정 시간표와 로컬 알림, 역 탭은 검색·가까운 역·즐겨찾기·최근 역을 담당합니다. 미스트 블루
바탕과 기능별 파스텔 카드는 밝은 인상을 유지하되 상태와 기능을 색으로 구분합니다.
...
- GPS 좌표는 iPhone 내부에서 가까운 역을 계산할 때만 사용하며 Worker로 보내지 않는다.
- Worker에는 선택 역명·노선명·날짜만 HTTPS로 전송한다. 앱은 Wi-Fi 또는 이동통신 데이터를
  사용하므로 오프라인 전용 앱은 아니다.
- 서울시·공공데이터포털 원본 키는 Worker Secret에만 있고 앱에는 개인용 Bearer 토큰만 들어간다.
- Bearer 토큰은 공개 앱의 사용자 인증이 아니므로 개인 설치 범위를 벗어나 배포하기 전에는
  기기별 인증 설계를 다시 승인한다. 현재 토큰은 필요할 때 값 비공개 방식으로 회전한다.
- Worker는 고정 경로·입력 allowlist, 경로별 속도 제한, 짧은 응답 캐시를 사용하며 invocation
  logging은 꺼 둔다. Cloudflare Workers Free 플랜은 한도 초과 과금 대신 요청을 차단하므로
  자동 유료 비용은 발생하지 않는다.
...
판정, 영업일 시계, 00·24시 파싱, 막차 DTO와 마지막 정상 시간표 유지, 즐겨찾기·최근 역 저장
검증을 포함합니다. Xcode 26.6의 iPhone 17 Pro Simulator에서 2026-08-22 전체 47개가
통과했습니다.
...
확인했습니다. 실제 네트워크 실패 UI와 실기기 검증은 승인표의 미완료 항목으로 남아 있습니다.
같은 Simulator에서 Morning Transit 네 탭, 강남역 검색·선택·홈 복귀, 실제 도착·위치 데이터
표시를 Computer Use로 확인했습니다. 디자인 승인 검증 내역은
[`Docs/MORNING_TRANSIT_ACCEPTANCE.md`](Docs/MORNING_TRANSIT_ACCEPTANCE.md)에 정리했습니다.

2026-08-21에는 Simulator에서 `오늘`, `평일`, `토요일`, `일·공휴일`을 모두 전환하고 각 시간표가
갱신되는 것을 확인했습니다. 공휴일 키 미설정 경고와 요일 폴백도 실제 화면에서 확인한 뒤,
공공데이터포털 키를 기존 Free 플랜 Worker에 반영했습니다. 비밀값을 출력하지 않는 종단간
점검에서 2026-08-21은 `weekday`, 2026-08-15는 `sunday_holiday / 광복절`로 확인했습니다.
접근성 대형 글자에서는 상단 상태, 섹션 제목과 막차 행이 세로 레이아웃으로 전환되도록 보강했습니다.
강남 2호선과 고속터미널 3호선은 Worker 원본을 앱과 같은 규칙으로 축약해 각각 7개 표시 행을
대조했습니다.
